...
namespace Lab5test
{
    ...
    public sealed class GreenTest
    {
        ...
        {
            // Arrange
            var input1 = _data.GetMatrixes();
            var input2 = new int[10] { 1, 2, 3, 4, 5, 0, 1, 3, 5, 7 };
            var answer = new int[][,] {
                new int[,] {
                    {1, 2, 3, 4, 5, 6, 7},
                    {5, 6, 7, 8, 9, 10, 11},
            ...
            for (int i = 0; i < answer.Length; i++)
            {
                _main.Task3(input1[i], input2[i]);
            }
            // Assert
            for (int i = 0; i < answer.Length; i++)
            {
                Assert.AreEqual(answer[i].GetLength(0), input1[i].GetLength(0));
                for (int j = 0; j < answer[i].GetLength(0); j++)
                {
                    Assert.AreEqual(answer[i].GetLength(1), input1[i].GetLength(1));
                    for (int k = 0; k < answer[i].GetLength(1); k++)
                    {
                        Assert.AreEqual(answer[i][j, k], input1[i][j, k]);
                    }
                }
            }
        }
